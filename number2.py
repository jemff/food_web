...
def lemke_optimizer(eco, payoff_matrix = None, dirac_mode = True):
    ...
    for k in range(eco.populations.size):
        A[k, k * eco.layers:(k + 1) * eco.layers] = -1*eco.spectral.M.sum(axis = 1)

    q = np.zeros(eco.populations.size + eco.populations.size * eco.layers)
    q[eco.populations.size * eco.layers:] = -1
    q = q.reshape(-1, 1)
    if payoff_matrix is None:
        payoff_matrix = total_payoff_matrix_builder(eco, dirac_mode = True)
    ...
    options.dparam[sn.SICONOS_DPARAM_TOL] = 10**(-8)
...
norm_dist = stats.norm.pdf(obj.x, loc=0, scale=4)
...
for i in range(total_time_steps):
    ...
    current_foraging = foraging_gain_builder(eco, dirac_mode = True)
...
with open('eco_dyn_res_lemke_heat.pkl', 'wb') as f:
    pkl.dump(eco, f, pkl.HIGHEST_PROTOCOL)

with open('strategies_eco_dyn_res_lemke_heat.pkl', 'wb') as f:
    pkl.dump(strategy_list, f, pkl.HIGHEST_PROTOCOL)

with open('population_eco_dyn_res_lemke_heat.pkl', 'wb') as f:
    pkl.dump(population_list, f, pkl.HIGHEST_PROTOCOL)

with open('resource_eco_dyn_res_lemke_heat.pkl', 'wb') as f:
    pkl.dump(resource_list, f, pkl.HIGHEST_PROTOCOL)

with open('rewards_eco_dyn_res_lemke_heat.pkl', 'wb') as f:
    pkl.dump(reward_t, f, pkl.HIGHEST_PROTOCOL)

with open('losses_eco_dyn_res_lemke_heat.pkl', 'wb') as f:
    pkl.dump(loss_t, f, pkl.HIGHEST_PROTOCOL)

with open('periodic_layers_eco_dyn_res_lemke_heat.pkl', 'wb') as f:
