...
\end{figure}
The driver of the daily production and growth cycles in the system with full rationality or bounded rationality is the vertical migration. Examining snapshots of the distributions, at midnight, noon, and dusk allows for a greater understanding. At midnight \Cref{fig:specific_dists_rational}(1) the consumers are concentrated near the surface, with a discontinuous drop to nothing. The predators follow the consumers, albeit with a continuous shape. At dusk, \Cref{fig:specific_dists_rational}(2) the predators have a greater concentration near the surface, while the consumer "box" is begining to form, yet still with a continuous drop-off due to the risk from the light. At noon, \Cref{fig:specific_dists_rational}(3) the consumers form a deep scattering layer, where most of the predators are also present, excepting a few hanging out higher in the water column deterring upward consumer migration \Cref{fig:specific_dists_rational}(3)(15 m).
\begin{figure}
