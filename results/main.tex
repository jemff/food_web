...
\begin{abstract}
  Game theory has emerged as an important tool to understand interacting populations in the last 50 years. The methods of game theory have been successfully applied to study population dynamics with optimal behavior in simple ecosystem models, but the methods are not   generalizable larger, more complex, systems. The usual application of game-theory for population models is in determining the patch distributions for optimal foraging, and coupling the instantaneous growth to population dynamics. It is common to simplify a complex habitat into just a few patches, but bopulations in the real world interact in continuous habitats, and in continuous time. In addition, the standing game-theoretic assumption of optimal behavior based on perfect information is a large simplification. We develop a game-theoretical method for determining the optimal patch choice for several interacting populations in continuous space. Going to a continuous setting allows us to model bounded rationality, and analyze the impact of bounded rationality on population dynamics.
  We use the method we develop to study a model of predator-prey interactions in continuous time and space, where the population dynamics are governed by the Nash equilibrium at every instant. Our model is that of an copepods interacting with forage fish, which gives rise to the largest animal migration in the world, the diel vertical migration. Incorporating both continuous space and time, we can model the seasonal variation in the migration, separating the effect of light and population numbers. This gives a qualitative agreement with empirical findings on the seasonal variation in the migration. Our method of including bounded variation gives rise to spatial distributions which correspond well to reality, while the population dynamics for bounded rationality and complete rationality are approximately equivalent. The essential ingredients in building the method are combining a result for the general resolution of polymatrix games, a spectral discretization scheme, and robust numerics for time-stepping.
\end{abstract}
\begin{comment}
\begin{abstract}
