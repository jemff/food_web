...
\begin{abstract}
  Game theory has emerged as an important tool to understand interacting populations in the last 50 years. The methods of game theory have been successfully applied to study population dynamics with optimal behavior in simple ecosystem models, but \replaced{existing}{the} methods are generally not applicable to larger, more complex, systems. In population models, game theory is usually applied to determine instantaneous optimal patch distributions, coupling behavior to population dynamics. Complex habitats are usually split into a few patches, but populations in the real world interact in continuous space and time. The assumption of decisions based on perfect information is a large simplification, but is hard to avoid. Here, we develop a game-theoretical method to study population dynamics for interacting populations in continuous space, with instantaneous optimal distribution. A continuous setting allows us to model bounded rationality, and compare the impact of bounded and complete rationality on population dynamics. \added{Noget om numerik her.} We \replaced{apply the method to an idealized case involving}{use the method to study} the population dynamics and \replaced{vertical distribution}{behavior} of forage fish preying on copepods. This interaction daily causes the largest animal migration \added{(Muligivs er denne oplysning overflødig?)} in the world, the diel vertical migration. Incorporating continuous space and time, we can model the seasonal variation in the migration, separating the effects of light and population numbers. We arrive at  qualitative agreement with empirical findings. Including bounded rationality gives rise to spatial distributions corresponding to reality, while the population dynamics for bounded rationality and complete rationality are equivalent. Our approach is general, and can easily be used for complex terrestrial and aquatic systems.

\todo[inline]{Et aspekt mangler: Det med den beregningsmæssige teknik til at opløse det diskretiserede spil. Så tilføj en sætning eller to om det. Til gengæld kan nogle af sætningerne kortes lidt ned, hvis man går dem igennem med en tættekam: Abstracts bør være så koncise som muligt. } 
\end{abstract}
...
\todo[inline]{Check bindestreger - blandt keywords er kun ``predator-prey'' kosher; de andre bruges ikke. Optræder også i selve teksten.}
