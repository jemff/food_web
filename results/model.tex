...
\todo[inline]{Jeg har vist indført lidt inkonsistent notationen her ...}
\[
  \ip{s_p}{e_p} = 
  \ip{s_c}{e_c} = 
  1
\]
since these mixed strategies are probability distributions on strategy space. Here $e_c$ and $e_p$ are the ``one'' functions\todo{Or vectors? Clarify setting.}. In addition the Nash equilibrium dictates
\[
