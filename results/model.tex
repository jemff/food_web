...
%The algorithm in \citep{miller1991copositive} has the problem that when the number of players grows very large, solving the problem becomes harder and harder due to the fullness of the matrix. As all interactions between species have a maximal payoff different from zero, we can get away with only subtracting the maximum from the non-zero entries, thereby getting a sparse if we consider a wide food-web.
