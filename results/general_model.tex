...
consider a Lotka-Volterra model with $M$ patches and $T$ types. Assume the interactions of animals of type $i$ with type $j$ is given by a matrix $A_{ij}$, describing the inter-patch interactions. The intrinsic growth of type of $i$ at patch $k$ is given by $G_{i}(k)$. Assuming that the populations are distributed according to $(p_i)_{i=1}^N$ the population dynamics of type $i$ with total abundance $N_i$ become:
  ...
  \label{eq:pop_dyn_lv}
\end{equation}
We define the fitness proxy for an individual of type $i$ at patch $k$ by $F_i(k)$. Remark that if an individual of type $i$ at patch $k$ interacts with other individuals of type $i$ each individual gets half the total payoff \Cref{kvrivan2008ideal}, which we indicate by $v_{ij}$.
\begin{equation}
...
\begin{equation}
  F_i(\phi_i, (N_j \phi_j)_{j=1}^T) = \ip{p_i}{\sum_{j=1}^T E_{ij} p_j}
\end{equation}
