...
\subsection*{General game-theoretic model}
The ideal free distribution is a general model that explains a wide range of habitat choice.. .... Models where the
An example of such a game is a habitat selection game, where picking the strategy that maximizes the instantaneous growth gives rise to an ESS \citep{kvrivan2009}. Cressman 2010, kirvan2008
...
\begin{equation}
 \dot{N_i} = N_i U_i
\end{equation}
A solution of a polymatrix game describing patch-selection is the multi-species ideal free distribution for a multiple patch-choice model. The patch-choice determines the instantaneous growth of each individual.
Generally assume migration dynamics much faster than the population time-scale.
The patch-choice model is often a necessary simplification of a continuous habitat, algorithms akin to ours have been used before, but only single-species and not with discretization (patrizio et al. )
%\todo[inline]{Hvis det her ``kun'' er habitatsselektion, så kan vi lige så godt skrive det. Hvad kan det ellers være?}


The basic structure in the games we study is a system of $N$ players, or species, with strategies at each point in time given by probability distributions $\phi_i(x)$ on an interval $[0,z_0]$. In a polymatrix game the payoffs $U_i$ of the players are given by sums of bilinear forms determined by matrices $A_{ij}$, replaced by linear operators $U_{ij}$ in the continuous setting. The total payoff to player $i$ in the continuous setting is:
...
\end{equation}
In the case where the interactions are purely local, $U_{ij}$ is a function $U_{ij}(x)$.

We can see that this generalizes the notion of a polymatrix game \citep{howson1972equilibria}, if the integral is replaced by a sum and the probability distributions are replaced by strategy vectors.
