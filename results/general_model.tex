...
\begin{equation}
  ...
  \label{eq:utility_pm}
\end{equation}
If migrations are very fast and the habitat is highly interconnected \citep{abrams2007role}, it is reasonable to assume all animals of any type simultaneously seek to find the optimum patch in the sense of seeking $k$ to maximize \Cref{eq:utility_pm}. A population which follows the optimal strategy for patch selection at any instant cannot be invaded by mutants following other strategies, so it constitute an evolutionarily stable strategy \citep{kvrivan2009evolutionary}. Population games, i.e. \Cref{eq:pop_dyn_lv} where the distribution is given by finding the best patch via \Cref{eq:utility_pm}, is a successful approach to coupling optimal behavior with population dynamics \citep{kvrivan2008ideal,morris2020time,mougi2019adaptive}.
