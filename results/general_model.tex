...
consider a Lotka-Volterra model with $M$ patches and $T$ types. Assume the interactions of animals of type $i$ with type $j$ is given by a matrix $A_{ij}$, describing the inter-patch interactions. The intrinsic growth of type of $i$ at patch $k$ is given by $G_{i}(k)$. Assuming that the populations are distributed according to $(p_i)_{i=1}^N$ the population dynamics of type $i$ with total abundance $N_i$ become:
  ...
  \label{eq:pop_dyn_lv}
\end{equation}
We define the fitness proxy for an individual of type $i$ at patch $k$ by $F_i(k)$. Remark that if an individual of type $i$ at patch $k$ interacts with other individuals of type $i$ each individual gets half the total payoff, which we indicate by $v_{ij}$.
\begin{equation}
  F_i(\phi_i, (N_j \phi_j)_{j \leq T})(k) = p_i(k) \pa{\sum_{j=1}^T {v_{ij}A_{ij}N_j p_j}(k) + G_{i}(k)}, \quad v_{ij}=1,i\neq j,~v_{ij}=\frac{1}{2},i=i
  \label{eq:utility_pm}
\end{equation}
...
\begin{equation}
  ...
  \begin{split}
  \Gamma_i(k,l) = G_i(l) \\
    E_{ij} = v_{ij}N_j A_{ij} + \frac{1}{T-1}\Gamma_i
  \end{split}
\end{equation}
