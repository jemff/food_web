...
To understand \added{?} the intrinsic coupling of patch-choice models with population dynamics,
consider a Lotka-Volterra model\todo{Uklart om dette \emph{er} ``the general model'', eller et eksempel på hvad en model kunne være.} with $M$ patches and $T$ types. Assume the interactions of animals of type $i$ with type $j$ is given by a matrix $A_{ij}$, describing the inter-patch interactions. The intrinsic growth of type $i$ at patch $k$ is given by $G_{i}(k)$. Assuming that the populations are distributed according to $(p_i)_{i=1}^N$ \todo{Her bør det uddybes at $p_i$ er en sandsynlighedsfordelingen på $\{1,\ldots,M\}$; evt. indføre $p_{ik}$ som sandsynligheden for at et dyr af type $i$ vælger patch $k$.}the population dynamics of type $i$ with total abundance $N_i$ become:
  \begin{equation}
  \dot{N}_i = N_i \sum_{k=1}^M p_i(k) \pa{\sum_{j=1}^T {(A_{ij}N_j) p_j}(k) + G_i(k)} %, ~i\neq
  \label{eq:pop_dyn_lv}
\end{equation}
\todo[inline]{Jeg forstår stadig ikke modellen :) $A$ er bare en matriks med elementer $A_{ij}$, ikke? Så er der ingen inter-patch interaktioner, men kun intra-patch, inter-type? Hvorfor parantesen omkring $(A_{ij}N_j)$? Hvorfor notationen $G_i(k)$ i stedet for $G_{ik}$? Det er vel for at forberede læseren på det kontinuerte tilfælde, hvor $k\in\{1,\ldots,M\}$  bliver erstattet af $x\in [0,1]$, men den kan læseren vel ikke forventes at forudse på dette tidspunkt?}

We define the fitness proxy for an individual of type $i$ at patch $k$ by $H_i(k)$.
\todo[inline]{forklar at denne "fitness proxy" er den specifikke vækstrate, som et dyr af type $i$ ville kunne opnå ved at vælge patch $k$, forudsat at fordelingerne er som angivet af $p$. Jeg forstår ikke notationen i venstresiden.}
\begin{equation}
  H_i(p_i, (N_j p_j)_{j=1}^T)(k) = \pa{\sum_{j=1}^T {A_{ij}N_j p_j}(k)} + G_i(k)%
  \label{eq:utility_pm}
\end{equation}
If migrations are very fast and the habitat is highly interconnected \citep{cressman2006migration, abrams2007role}, it is reasonable to assume all animals of any type simultaneously seek to find the optimum patch in the sense of seeking $k$ to maximize \Cref{eq:utility_pm}. In a single-species case, this is the ideal free distribution \citep{fretwell1969territorial}. \todo{Her bør løsningens struktur genopfriskes: At det betyder at $p$ bliver valgt sådan et $H_i$ er konstant, undtagen hvor $p_{ik}$ er 0, etc.} The approach of using population dynamics determined by \Cref{eq:pop_dyn_lv} with optimal strategies determined by the Nash equilibrium defines a population game, and is a successful approach to coupling optimal behavior with population dynamics \citep{valdovinos2010consequences, mougi2019adaptive, pinti2021co}.
...
Changing to a polymatrix game reveals a clear connection between the individual game and the population growth rate, \Cref{eq:poly_fitness} is the growth rate \Cref{eq:pop_dyn_lv} with $A_{ii}$ scaled by $\frac{1}{2}$. Therefore, optimizing the population growth per type and at the individual level are the same when there is no density dependence, but otherwise subtly different. In the case where \Cref{eq:utility_pm} has no density dependence following the Nash equilibrium at every instant is an evolutionary stable strategy, i.e. the populations cannot be invaded by mutants \citep{kvrivan2009evolutionary}. These considerations resolve the discrete case, but in nature many habitats are continuous and cannot be described well as discrete patches. Our approach provides a way to resolve population games while keeping the continuous nature of the habitat.
