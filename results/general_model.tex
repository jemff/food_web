...
\end{equation}
We can construct $E_{ij}$ explicitly if define $\Gamma_i$ as a matrix where every column is $G_i$, so $(\Gamma_i p_j)(k) = G(k)$. The total payoff matrices $E_{ij}$ for the interaction between type $i$ and $j$ in \Cref{eq:poly_fitness} then become:
\begin{equation}
