...
\begin{equation}
  ...
  \end{split}
\end{equation}
By the polymorphic-monomorphic equivalence, finding $k$ to simultaneously maximize \Cref{eq:utility_pm} is equivalent to maximizing the monomorphic fitness:
\begin{equation}
  \label{eq:poly_fitness}
  F_i(p_i, (N_j p_j)_{j=1}^T) = \ip{p_i}{\sum_{j=1}^T E_{ij} p_j}
\end{equation}
for all $i$ simultaneously, that is, finding the Nash equilibrium in a polymatrix game. We see that \Cref{eq:poly_fitness} is the growth rate \Cref{eq:pop_dyn_lv} with $N_{ij}$ scaled by $\frac{1}{v_{ij}}$. Therefore, populations where \Cref{eq:utility_pm} has no density dependence and where the distributions follow the Nash equilibrium cannot be invaded by mutants \citep{kvrivan2009evolutionary} i.e. the populations follow an evolutionarily stable strategy. These considerations resolve the discrete case, but in nature many habitats are continuous and cannot be described well as discrete patches. Our approach provides a way to resolve population games while keeping the continuous nature of the habitat.

The insight of using the monomorphic-polymorphic equivalence is essential in generalizing to the continuous case, since it highlights that the important factor in the individual patch choice \Cref{eq:utility_pm} is actually the overall distribution on patches $p_i$. To extend population games to continuous space and facilitate the incorporation of imperfect decision making, we consider a habitat described by an interval $[0,z_0]$. We again assume we have $T$ different types. Define the continuous analogues of patch distribution by:
\begin{equation}
