...
\todo[inline]{Jeg ville lade afsnittet begynde her og flytte det overstående til introduktionen.}

The basic structure in the games we study is a system of $N$ players, with strategies given by probability distributions $\phi_i$ on a space $X$, henceforth an interval $[0,z_0]$. The payoffs $U_i$ of the players are given by matrices, or, in the continuous setting, linear operators $A_{ij}$ as\todo{Bør vi ikke nævne begrebet ``habitat selection''? Kan modellen beskrive andet end habitatsselektion?}
\begin{equation}
  \label{eq:utility}
