...
If we assume that $A_{ii}=0$ for all $i$, the problem of finding the Nash equilibrium $(p_i^{*,NE})_{i=1}^T$ of the game specified by all $H_i$ in \Cref{eq:utility_pm} can be interpreted as finding the Nash equilibrium in a polymatrix game \citep{howson1972equilibria, eaves1973polymatrix}. %The interpretation comes from noting that $H_i$ is affine in $p_j$ and independent of $p_i$, so the sum of the payoff at all patches can be written as
%\begin{equation}%
