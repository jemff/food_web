...
\begin{equation}
  U_i(\phi_i, (N_j \phi_j)_{j \leq N, j \neq i}) = \sum_{j=1,j\neq i}^N \int \phi_i(x) (U_{ij}(t)N_j \phi_j)(x) dx%, ~i\neq j~n_{ij}=1,~i=j~n_{ij} = \frac{1}{2}
  \label{eq:utility}
\end{equation}
In the case where the interactions are purely local, $U_{ij}(t)$ is a function $U_{ij}(t)(x)$. The habitat selection game is given by each animal maximizing the functional in \Cref{eq:utility} simultaneously, since the game again has polymorphic-monomorphic equivalence.
